...
type CameraView = 'birdseye' | 'street' | 'aerial' | 'custom';
...
export class MapApp extends LitElement {
  ...
  @state() mapError = '';
  @state() activeCameraView: CameraView = 'custom';
  ...
  // Flag to prevent camera-change event from resetting the active view when we are setting it programmatically
...
You can find this constant near the top of the map_app.ts file.`;
    ...
    try {
      // FIX: The 'load' method is deprecated on the Loader. Use 'importLibrary'
      // to load specific libraries, which also handles loading the Maps JS API.
      await Promise.all([
  ...
  private handleCameraChange() {
    if (this.isSettingView) {
      // This change was initiated by our button click, so don't reset the active view.
      return;
    }
    this.activeCameraView = 'custom';
  }

  async setCameraView(viewType: 'birdseye' | 'street' | 'aerial') {
    if (!this.map) return;

    this.isSettingView = true;
    this.activeCameraView = viewType;

    const currentCameraState = (this.map as any).cameraState;
    const center = currentCameraState.center;
    const heading = currentCameraState.heading;
    ...
    switch (viewType) {
      case 'birdseye':
        newCameraOptions = {
          ...
          heading: 0,
          tilt: 0,
          range: currentCameraState.range,
        };
        break;
      case 'street':
        newCameraOptions = {
    ...
    (this.map as any).flyCameraTo({
      endCamera: newCameraOptions,
      durationMillis: durationMillis,
    });

    // Reset the flag after the camera transition is likely complete.
    setTimeout(() => {
  ...
  render() {
    ...
    return html`<div class="gdm-map-app">
      <div
        ...
        <div class="camera-controls" role="toolbar" aria-label="Camera Views">
          <button
            class=${classMap({
              active: this.activeCameraView === 'birdseye',
            })}
            @click=${() => this.setCameraView('birdseye')}
            ...
            <svg
              ...
              width="24px">
              <path
                d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm0-80h560v-560H200v560Zm280-120q-66 0-113-47t-47-113q0-66 47-113t113-47q66 0 113 47t47 113q0 66-47 113t-113 47Zm0-80q33 0 56.5-23.5T560-480q0-33-23.5-56.5T480-560q-33 0-56.5 23.5T400-480q0 33 23.5 56.5T480-400Zm0-80Z" />
            </svg>
          </button>
          <button
            class=${classMap({
              active: this.activeCameraView === 'aerial',
            })}
            @click=${() => this.setCameraView('aerial')}
            title="Aerial View"
            aria-label="Switch to Aerial View">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 -960 960 960"
              width="24px">
              <path
                d="m480-240-160-80v-240L160-440v280l320 160Zm0 80L160-360v-360l320 160 320-160v360L480-160Zm160-360L480-440v240l160 80v-280ZM383-480l-43-22 140-70 43 22-140 70Zm237 118L480-292l-140-70 140-70 140 70Z" />
            </svg>
          </button>
          <button
            class=${classMap({
              active: this.activeCameraView === 'street',
            })}
            @click=${() => this.setCameraView('street')}
            title="Street Level View"
            aria-label="Switch to Street Level View">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
              viewBox="0 -960 960 960"
              width="24px">
              <path
                d="M440-360v-280q0-17 11.5-28.5T480-680q17 0 28.5 11.5T520-640v280h80v-440q0-17 11.5-28.5T640-800q17 0 28.5 11.5T680-760v320h80v-240q0-17 11.5-28.5T800-720q17 0 28.5 11.5T840-680v560h-80v-240H600v240h-80v-360h-80v360H120v-520h80v440h160v-200q0-17 11.5-28.5T400-520q17 0 28.5 11.5T440-480v120Z" />
            </svg>
          </button>
        </div>
        ${
