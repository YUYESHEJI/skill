...
import { LaapDock } from "./laap-dock";
import type { LaapAtomicComponent } from "./ipc";
...
export default function App() {
  ...
  const [laapCockpitOpen, setLaapCockpitOpen] = useState(false);
  ...
  const toggleCockpit = useCallback(() => {
    setLaapCockpitOpen((current) => !current);
  }, []);

  const handleInsertComponent = useCallback((component: LaapAtomicComponent) => {
    setComposerDraft((current) => {
      const template = (component.template ?? component.prompt ?? "").trim();
      if (!template) {
        return current;
      }
      const block = `[${component.name}] ${template}`;
    ...
    return (
      <div className="shell shell--loading">
        <main className="loading-card">
          <div className="loading-card__eyebrow">LAAP</div>
  ...
  const mainClassName = [
    ...
    laapCockpitOpen ? "main--with-cockpit" : "",
  ].filter(Boolean).join(" ");
  ...
  const shellClassName = [
    ...
    laapCockpitOpen ? "shell--cockpit-open" : "",
  ].filter(Boolean).join(" ");
  ...
  return (
    <div className={shellClassName}>
      ...
      {laapCockpitOpen ? <LaapDock api={api} onInsertComponent={handleInsertComponent} onToggleCockpit={toggleCockpit} /> : null}
      ...
      <main className={mainClassName}>
        <Topbar
          ...
          cockpitVisible={laapCockpitOpen}
                ...
                {showSchemaSkewNotice ? (
                  <div className="schema-skew-notice" role="status" data-testid="schema-skew-notice">
                    <span className="schema-skew-notice__text">
                      This session was written by a newer version of LAAP — some content may not display. Update LAAP
                      (or open it with the LAAP CLI) to see everything.
                    </span>
                    <button
                      type="button"
