...
export class DesktopAppStore implements AppStoreInternals {
  ...
  private async refreshRuntimeForAllWorkspaces(
    ...
    snapshots.forEach((result, index) => {
      ...
      console.warn(
        `[laap] Failed to refresh runtime for ${workspace?.path ?? "unknown workspace"} after custom provider update: ${
  ...
  private async refreshSessionCommandsForAllWorkspaces(): Promise<void> {
    ...
    results.forEach((result, index) => {
      ...
      console.warn(
        `[laap] Failed to refresh session commands for ${workspace?.path ?? "unknown workspace"} after custom provider update: ${
          result.reason instanceof Error ? result.reason.message : String(result.reason)
        }`,
      );
    });
  ...
  private async refreshStateNow(options: RefreshStateOptions): Promise<DesktopAppState> {
    ...
    try {
      ...
      secondaryRuntimeLoads.forEach((result, index) => {
        ...
        console.warn(
          `[laap] Failed to preload runtime for ${failedWorkspace?.path ?? "unknown workspace"}: ${
            result.reason instanceof Error ? result.reason.message : String(result.reason)
          }`,
        );
      });
  ...
  finishRuntimeCommandExecution(
    ...
    if (!pending.blockedMessage) {
      recordLearnedCommandCompatibility(this.extensionCommandCompatibilityByWorkspace, sessionRef.workspaceId, {
        ...
        status: "supported",
        message: "Observed working in LAAP.",
        capability: "gui-safe",
        updatedAt: timestamp,
      });
    }
  ...
  private reportExtensionCompatibilityIssue(
    ...
    const pending = this.pendingRuntimeCommandsBySession.get(key);
    if (pending) {
      const message = `/${pending.command.name} requires terminal-only ${formatCapabilityLabel(issue.capability)} and is not supported in LAAP yet. Use LAAP in the terminal for this command.`;
...
function describeStoreError(error: unknown): string {
  if (isSessionLeasedError(error)) {
    const { holder } = error;
    const where = holder.surface === "pi-cli" ? "the LAAP CLI" : "another LAAP instance";
