...
import {
  ...
  type TerminalSize,
  type LaapPersonaData,
  type LaapCognitionStatus,
...
contextBridge.exposeInMainWorld("piApp", {
  ...
  // ── LAAP 认知层（aris / LAAPer） ──
  getLaapPersonas: () => ipcRenderer.invoke(desktopIpc.laapGetPersonas) as Promise<LaapPersonaData>,
  setLaapPersona: (name: string) => ipcRenderer.invoke(desktopIpc.laapSetPersona, name) as Promise<LaapPersonaData>,
  createLaapPersona: (input: LaapCreatePersonaInput) =>
    ipcRenderer.invoke(desktopIpc.laapCreatePersona, input) as Promise<LaapPersonaData>,
  getLaapCognition: () => ipcRenderer.invoke(desktopIpc.laapGetCognition) as Promise<LaapCognitionStatus>,
  setupLaapModels: () => ipcRenderer.invoke(desktopIpc.laapSetupModels) as Promise<LaapSetupModelsResult>,
  getLaapRsiStatus: () => ipcRenderer.invoke(desktopIpc.laapRsiStatus) as Promise<LaapRsiStatus>,
  evolveLaapRsi: (component?: string) =>
    ipcRenderer.invoke(desktopIpc.laapRsiEvolve, component) as Promise<LaapRsiEvolveResult>,
  listLaapComponents: () => ipcRenderer.invoke(desktopIpc.laapComponentsList) as Promise<LaapComponentsListResult>,
  compileLaapComponent: (component: string) =>
    ipcRenderer.invoke(desktopIpc.laapComponentCompile, component) as Promise<LaapComponentCompileResult>,
});
